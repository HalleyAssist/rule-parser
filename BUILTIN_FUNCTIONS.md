...
- [Value Wrapper](#value-wrapper)
  - [Value](#valueliteralvalue)
  - [Array](#arrayarg1-arg2-)
  - [ArrayIn](#arrayinhaystack-needle)
...
Constructs an array from its arguments at runtime. Unlike `Value`, which wraps constant arrays, `Array` builds arrays dynamically from expressions that may include function calls or other non-constant values.

**Arguments:**
- `arg1, arg2, ...`: One or more expressions that will become array elements

**Use Cases:**
- Building arrays that include function call results
- Creating arrays with computed values
- Mixed constant and non-constant array elements
...
// Mixed constants and function calls
["Array", ["Value", 1], ["GetCount"], ["Value", 3]]

// Array with expressions
["Array", ["MathAdd", ["A"], ["Value", 1]], ["B"]]
```

**Difference from Value:**
- `["Value", [1, 2, 3]]` - constant array, known at parse time
- `["Array", ["Value", 1], ["A"], ["Value", 3]]` - dynamic array, may include function calls

**Note:** `Array` is not directly callable in rule syntax but is generated by the parser when needed (e.g., in `IN` expressions with non-constant values).

---

### ArrayIn(haystack, needle)

Checks if a value (needle) exists in an array (haystack). This is the membership operator used by the `IN` syntax.

**Arguments:**
- `haystack`: The array to search in. Can be:
  - `["Value", [array]]` when all elements are constants
  - `["Array", arg1, arg2, ...]` when elements include non-constants
- `needle`: The value to search for

**Examples:**
```javascript
// All constant values - uses Value wrapper
["ArrayIn", ["Value", ["a", "b", "c"]], ["Status"]]

// Non-constant values - uses Array function
["ArrayIn", ["Array", ["A"], ["B"], ["C"]], ["Status"]]

// Mixed constants and non-constants - uses Array function
["ArrayIn", ["Array", ["Value", "active"], ["GetStatus"], ["Value", "pending"]], ["CurrentStatus"]]
```

**Rule Syntax:**
```
Status() IN ("a", "b", "c")           // All constants
Status() IN (A(), B(), C())           // Function calls
Status() IN ("active", GetStatus())   // Mixed
```

**Optimization:**
When all haystack values are constants (literals), the parser automatically wraps them in a single `["Value", [array]]` for efficiency. When any value is non-constant (function call, expression), the parser uses `["Array", ...]` to construct the array at runtime.
